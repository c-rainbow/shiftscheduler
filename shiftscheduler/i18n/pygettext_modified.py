...
##              file=sys.stderr)
    ...
    def write(self, fp):
        # Do nothing if there is no messsage to translate
...
def main(input_filepath, output_filepath):
    ...
    class Options:
        ...
        escape = 0
        # In Windows, the default encoding for open() is cp949,
        # which is different from sys.getdefaultencoding() or any other system encoding.
        to_code = 'utf8'
        keywords = []
        outpath = ''
    ...
    if options.verbose:
        print(_('Working on %s') % input_filepath)
    
    fp = open(input_filepath, 'rb')
    try:
        eater.set_filename(input_filepath)
        try:
            ...
            print('%s: %s, line %d, column %d' % (
                e.args[0], input_filepath, e.args[1][0], e.args[1][1]),
                file=sys.stderr)
    finally:
        fp.close()

    # write the output
    if not eater.has_messages():
        return

    output_filepath.parent.mkdir(parents=True, exist_ok=True)
    fp = open(output_filepath, 'w', encoding=options.to_code)
    try:
        eater.write(fp)
    finally:
        fp.close()
