...
def ValidateWorkDateColumns(assignment_dict, start_date, end_date, errors):
...
def ValidateConstraint2(all_dates, person_configs, assignment_dict, errors):
...
def ValidateConstraint3(all_dates, person_configs, assignment_dict, errors):
    for person_config in person_configs:
        name = person_config.name
...
def ValidateConstriant4(all_dates, person_configs, assignment_dict, errors):
    for person_config in person_configs:
        name = person_config.name
...
def ValidateConstraint5(all_dates, person_configs, assignment_dict, errors):
    for person_config in person_configs:
        name = person_config.name
...
def ValidateTimetable(software_config, date_configs, person_configs, assignment_dict, barebone=False):
    ...
    # 1. Basic checks on work dates
    ValidateWorkDateColumns(assignment_dict, software_config.start_date, software_config.end_date, errors)
    ...
    # 5-2. No day/evening work after night shift
    ValidateConstraint2(all_dates, person_configs, assignment_dict, errors)

    # 5-3. No more than n consecutive workdays
    ValidateConstraint3(all_dates, person_configs, assignment_dict, errors)   

    # 5-4. No more than n consecutive nights
    ValidateConstriant4(all_dates, person_configs, assignment_dict, errors)       
    
    # 5-5. min_total_workdays <= total schedule <= max_total_workdays
    ValidateConstraint5(all_dates, person_configs, assignment_dict, errors)
    ...
    off_count = len([c for c in assignment_dict.values() if c == _OFF_SHIFT])
